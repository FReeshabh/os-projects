...
void show_help()
{
    ...
    printf("-n <threads> Specify the number of threads . (default=1)\n"); //THREAD INFO HERE
...
int main(int argc, char* argv[])
{
    ...
    int max = 1000;
    int num_threads = 1; //Number of threads, defaulted to 1
    ...
    for (int i = 0; i < num_threads; i++){
        ...
        args[i].start_height = (image_height/num_threads*i)+1;
        args[i].end_height   = (image_height/num_threads)*(i+1)+1;
        // args[i].start_width  = (image_width/num_threads*i)+1;
        // args[i].end_width    = (image_width/num_threads)*(i+1)+1;

        // int pthread_create(pthread_t *thread, const pthread_attr_t *attr,
        // void *(*start_routine)(void*), void *arg);

        pthread_create(&workers[i], NULL, (void*) compute_image, (void*) &args[i]);
...
void * compute_image(struct thread_args *args)
{
    ...
    // For every pixel in the image...
        ...
        //Don't need both height and weight
    for (j = args->start_height; j < args->end_height; j++) {
        // for (i = args->start_width; i < args->end_width; i++) {
            for (i = 0; i < width; i++) {
